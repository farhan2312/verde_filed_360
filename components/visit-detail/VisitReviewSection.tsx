...
export function VisitReviewSection({
  ...
  return (
    <div className={`${box} ${reviewed ? "border-[#E4EFC9] bg-[#F6FBF6]" : "border-[#F5CE8E] bg-[#FFFDF7]"}`}>
      <div className="mb-2 flex items-center gap-2">
        <span className="text-[14px] font-bold text-[#1A1C1A]">Visit review</span>
        <span className="rounded-full px-2 py-0.5 text-[10.5px] font-bold"
          style={{ background: reviewed ? "#F3F8E6" : "#FFF3E0", color: reviewed ? "#678722" : "#E65100" }}>
      ...
      {reviewed && !editing && (
        <>
          {reviewNote
            ? <div className="rounded-[10px] bg-white/70 px-3.5 py-2.5 text-[13px] leading-relaxed text-[#33691E] ring-1 ring-[#F3F8E6]">{reviewNote}</div>
          ...
          {canReview && (
            <div className="mt-3 flex gap-2">
              <button type="button" onClick={() => { setNote(reviewNote); setEditing(true); }} className="rounded-[10px] border border-[#678722] px-3.5 py-1.5 text-[12px] font-semibold text-[#678722] hover:bg-[#F3F8E6]">Edit note</button>
      ...
      {canReview && (editing || !reviewed) && (
        <>
          ...
          <textarea value={note} onChange={(e) => setNote(e.target.value)} rows={3}
            ...
            className="w-full resize-y rounded-[10px] border border-[#E0E0E0] px-3.5 py-2.5 text-[13px] outline-none focus:border-[#678722]" />
          <div className="mt-2 flex gap-2">
            <button type="button" onClick={save} disabled={busy} className="rounded-[10px] bg-[#678722] px-4 py-2 text-[13px] font-bold text-white hover:bg-[#516A1B] disabled:opacity-50">{busy ? "Saving…" : reviewed ? "Save note" : "Mark reviewed"}</button>
